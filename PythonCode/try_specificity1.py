...
# load files
features_mat = scipy.io.loadmat('../Feature/chb'+case+'feature/SNchb'+case+'features3.mat')
labels_mat = scipy.io.loadmat('../Feature/chb'+case+'feature/SNchb'+case+'labels3.mat')
nsidx_mat = scipy.io.loadmat('../Feature/chb'+case+'feature/SNchb'+case+'nsidx3.mat')
sidx_mat = scipy.io.loadmat('../Feature/chb'+case+'feature/SNchb'+case+'sidx3.mat')
...
# seizure
seizure_segments_features = []
seizure_segments_labels = []
for i,val in enumerate(s_start):
    seizure_segments_features.extend(feature_inputs[(s_start[i]):(s_end[i])+1,:])
    seizure_segments_labels.extend(label_inputs[(s_start[i]):(s_end[i])+1])
...
# non-seizure
nonseizure_segments_features = []
nonseizure_segments_labels = []
for i,val in enumerate(ns_start):
    nonseizure_segments_features.extend(feature_inputs[(ns_start[i]):(ns_end[i])+1,:])
    nonseizure_segments_labels.extend(label_inputs[(ns_start[i]):(ns_end[i])+1])
...
for i, start_idx in enumerate(nonseizure_record_start_indices):
    ...
    print y_pred
    fa = [x for x, y in groupby(y_pred) if (sum(1 for i in y) > pattern_num and x==1)]
    num_fa = sum(fa)
    print "   fa: ",num_fa
    ns_count += len(test_label)
    if num_fa > 0:
        fa_rate += num_fa  # count of false alarms

# print results
print "number of non-seizures ", len(ns_start)
