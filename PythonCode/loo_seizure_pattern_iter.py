...
Parameters to set: case, pre_seizure, iter, multi, pattern
"""
import scipy.io
import numpy as np
...
accrs = []  # to store accuracy scores of each iteration
las = []  # to store latencies of each iteration
...
for j in range(iter):
    ...
    for i, onset_idx in enumerate(onset_indices):
        ...
        # testing and measuring accuracy and latency
        y_pred = clf.predict(X_test)
        # detect seizure alarms
...
print "average accuracy (sensitivity): ", np.mean(accrs)
